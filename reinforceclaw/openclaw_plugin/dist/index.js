...
const failedCaptures = /* @__PURE__ */ new Map();
const capturesInFlight = /* @__PURE__ */ new Set();
const capturedMessages = /* @__PURE__ */ new Map();
const capturedMessageIds = /* @__PURE__ */ new Map();
const capturedPollIds = /* @__PURE__ */ new Map();
const ambiguousMessageIds = /* @__PURE__ */ new Map();
const capturedResponses = /* @__PURE__ */ new Map();
const capturedPendingKeys = /* @__PURE__ */ new Map();
const handledOutbound = /* @__PURE__ */ new Map();
const handledInbound = /* @__PURE__ */ new Map();
const promptedPendingKeys = /* @__PURE__ */ new Map();
let bridgeSecret = "";
const adminLastRun = /* @__PURE__ */ new Map();
const SAFE_COMMANDS = /* @__PURE__ */ new Set(["good", "bad", "undo", "status"]);
const ADMIN_COMMANDS = /* @__PURE__ */ new Set(["train", "rollback", "reset", "on", "off"]);
const GOOD_ALIASES = /* @__PURE__ */ new Set([
  "good",
  "yes",
  "y",
  "+1",
...
const BAD_ALIASES = /* @__PURE__ */ new Set([
  "bad",
  "no",
  "n",
  "-1",
...
const MAX_PENDING_AGE_MS = 30 * 60 * 1e3;
const DEDUPE_AGE_MS = 2 * 60 * 1e3;
const MAX_TEXT_BYTES = 2e6;
const MAX_CONTEXT_MESSAGE_BYTES = 12e4;
const MAX_TRACKED_TURNS = 2e3;
const BRIDGE_TIMEOUT_MS = 15e3;
const TRUNCATION_MARKER = "\n...[reinforceclaw truncated oversized text]\n";
const DEFAULT_HOST = "http://127.0.0.1:8420";
const PERMANENT_CAPTURE_REJECTS = /* @__PURE__ */ new Set([400, 411, 413, 414, 415]);
function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}
function idOf(value, depth = 0) {
  if (depth > 4 || value === void 0 || value === null) return "";
...
function sessionKey(event, ctx) {
  return idOf(ctx?.sessionKey) || idOf(event?.sessionKey) || idOf(event?.conversationId) || idOf(event?.threadId) || idOf(event?.target) || channelOf(event) || "default";
}
function outboundMessageIdOf(...values) {
  for (const value of values) {
    const id = idOf(
      value?.messageId ?? value?.messageID ?? value?.providerMessageId ?? value?.clientMessageId ?? value?.id ?? value?.key ?? value?.targetMessageId ?? value?.targetId ?? value?.ts ?? value?.timestamp ?? value?.message ?? value?.targetMessage ?? value?.item ?? value?.callback
    );
    if (id) return id;
  }
  return "";
}
function reactionMessageIdOf(event, ctx) {
  return outboundMessageIdOf(
    event?.targetMessage ?? event?.message ?? event?.item ?? event?.callback,
    {
      messageId: event?.targetMessageId ?? event?.targetId ?? event?.reaction?.messageId ?? event?.reaction?.targetMessageId ?? ctx?.messageId ?? ctx?.callback?.messageId
    }
  );
}
function outboundMessageKey(event, ctx) {
  const id = outboundMessageIdOf(event, ctx?.message, ctx?.callback);
  return id ? `${sessionKey(event, ctx)}#${id}` : "";
}
function reactionMessageKey(event, ctx) {
  const id = reactionMessageIdOf(event, ctx);
  return id ? `${sessionKey(event, ctx)}#${id}` : "";
}
function contentOf(value, depth = 0) {
  if (depth > 4 || value === void 0 || value === null) return "";
...
function capText(text, maxBytes = MAX_TEXT_BYTES) {
  if (Buffer.byteLength(text, "utf8") <= maxBytes) return text;
  let lo = 0;
  let hi = text.length;
  while (lo < hi) {
    const keep = Math.ceil((lo + hi) / 2);
    const head2 = Math.floor(keep / 2);
    const candidate = `${text.slice(0, head2)}${TRUNCATION_MARKER}${text.slice(-(keep - head2))}`;
...
function cleanText(value) {
  return String(value || "").replace(/\ufe0f/g, "").trim().toLowerCase().replace(/^[`*_.!:\s]+|[`*_.!:\s]+$/g, "").replace(/\s+/g, " ");
}
function normalizeCommand(value, allowBare = true) {
...
function isReinforceCommandText(value) {
  const text = cleanText(value);
  return ["/rl", "/rc", "/reinforceclaw"].includes(text) || ["/rl ", "/rc ", "/reinforceclaw "].some((prefix) => text.startsWith(prefix));
}
function reactionOf(event) {
  const reaction = event?.reaction;
  if (typeof reaction === "string" || typeof reaction === "number") return String(reaction);
  return String(
    event?.emoji ?? event?.reactionName ?? reaction?.emoji ?? reaction?.name ?? reaction?.text ?? event?.name ?? contentOf(event)
  );
}
function pollIdOf(value) {
  return idOf(
    value?.pollId ?? value?.poll_id ?? value?.poll?.id ?? value?.poll?.pollId ?? value?.pollAnswer?.poll_id ?? value?.pollAnswer?.pollId ?? value?.poll_answer?.poll_id ?? value?.answer?.pollId ?? value?.answer?.poll_id
  );
}
function pollChoiceOf(event) {
...
function stopDelivery(event, ctx) {
...
function lowerField(obj, names) {
  for (const name of names) {
    const value = obj?.[name] ?? obj?.metadata?.[name];
    if (value !== void 0 && value !== null) return String(value).toLowerCase();
  }
  return "";
}
function trueField(obj, names) {
  return names.some((name) => obj?.[name] === true || obj?.metadata?.[name] === true);
}
function feedbackMode(cfg) {
  const raw = String(cfg.reinforceclawFeedbackMode || "slash_reactions_prompt").trim().toLowerCase().replace(/[-\s]+/g, "_");
...
function feedbackPromptsEnabled(cfg) {
  return feedbackMode(cfg) === "slash_reactions_prompt";
}
function roleOf(event, ctx) {
  return lowerField(event, ["role", "senderRole", "authorRole", "messageRole", "type"]) || lowerField(ctx, ["role", "senderRole", "authorRole"]);
}
function isHumanInbound(event, ctx) {
  ...
  if (trueField(event, ["bot", "isBot", "fromBot", "fromSelf", "outgoing", "system"]) || trueField(ctx, ["bot", "isBot", "fromBot", "fromSelf", "outgoing", "system"])) return false;
  return true;
}
function isAssistantOutbound(event, ctx) {
  const role = roleOf(event, ctx);
  const direction = lowerField(event, ["direction", "messageDirection"]);
  ...
  return trueField(event, ["fromSelf", "outgoing", "isAssistant", "isBot"]) || trueField(ctx, ["fromSelf", "outgoing", "isAssistant", "isBot"]);
}
function channelOf(event) {
  return idOf(event?.channelId) || idOf(event?.channel) || idOf(event?.metadata?.provider) || "openclaw";
}
function providerOf(event, ctx) {
  const raw = [
...
function outboundTarget(event, ctx) {
  return idOf(event?.to) || idOf(event?.target) || idOf(event?.conversationId) || idOf(ctx?.conversationId) || idOf(event?.channelId) || idOf(ctx?.channelId) || idOf(ctx?.threadId);
}
function accountIdOf(event, ctx) {
  return idOf(event?.accountId) || idOf(event?.metadata?.accountId) || idOf(ctx?.accountId);
}
function rolloutContext(event, ctx, prompt) {
  const raw = Array.isArray(ctx?.messages) ? ctx.messages : Array.isArray(event?.messages) ? event.messages : [];
  const messages = raw.map((m) => ({ role: String(m?.role || ""), content: capText(contentOf(m).trim(), MAX_CONTEXT_MESSAGE_BYTES) })).filter((m) => ["user", "assistant"].includes(m.role) && m.content).slice(-12);
...
function callerId(ctx) {
  return idOf(ctx?.userId) || idOf(ctx?.senderId) || idOf(ctx?.authorId) || idOf(ctx?.fromId) || idOf(ctx?.user) || idOf(ctx?.sender) || idOf(ctx?.author) || idOf(ctx?.from);
}
function actorId(event, ctx) {
  return callerId(event) || callerId(ctx) || idOf(event?.reactorId) || idOf(event?.actorId) || idOf(event?.user?.id);
}
function allowedRaters(cfg) {
  return String(cfg.reinforceclawRaterUsers || cfg.reinforceclawAdminUsers || "").split(",").map((x) => x.trim()).filter(Boolean);
}
function canRateTurn(turn, event, ctx, cfg) {
  const systemReaction = event?.reinforceclawReactionSystem === true;
  const blockedFields = systemReaction ? ["bot", "isBot", "fromBot", "fromSelf", "outgoing"] : ["bot", "isBot", "fromBot", "fromSelf", "outgoing", "system"];
...
function responseSig(response) {
  return `${response.length}:${response.slice(0, 256)}`;
}
function responseLookupKey(key, response) {
  return `${key}:${responseSig(response)}`;
}
function rememberCapture(key, turn, event, ctx) {
...
function capturedForResponse(base, userId, response) {
  ...
  const matches = [...capturedResponses.entries()].filter(([key]) => key.startsWith(`${base}::`) && key.endsWith(suffix)).map(([, turn]) => turn);
  return matches.length === 1 ? matches[0] : void 0;
}
function pendingFor(base, userId) {
  ...
  return matches.length === 1 ? { key: matches[0][0], user: matches[0][1][0] } : void 0;
}
...
function adminAllowed(sub, ctx, cfg) {
  if (!ADMIN_COMMANDS.has(sub) || ![true, "true", "1"].includes(cfg.reinforceclawAllowAdminCommands)) return false;
  const allowed = String(cfg.reinforceclawAdminUsers || "").split(",").map((x) => x.trim()).filter(Boolean);
  return allowed.length > 0 && allowed.includes(callerId(ctx));
}
function adminRateLimited(sub, ctx) {
  if (!ADMIN_COMMANDS.has(sub)) return false;
  const key = `${callerId(ctx)}:${sub}`;
  const now = Date.now();
  const last = adminLastRun.get(key) || 0;
  if (now - last < 3e4) return true;
...
function trimMap(map, max = MAX_TRACKED_TURNS) {
  if (map.size <= max) return;
  const rows = [...map.entries()].map(([key, value]) => [
    key,
    typeof value === "number" ? value : Number(value?.ts || 0)
  ]);
...
async function waitForBridge(host) {
  for (let i = 0; i < 10; i++) {
...
function fetchWithTimeout(url, init = {}, timeoutMs = BRIDGE_TIMEOUT_MS) {
...
async function command(host, body) {
  try {
...
var index_default = definePluginEntry({
  ...
  register(api) {
    ...
    const captureTurn = async (key, turn) => {
      try {
    ...
    const rateLatest = async (key, sub, pendingKey = "") => {
    ...
    const commandKey = (event, ctx) => scopedKey(sessionKey(event, ctx), callerId(ctx) || callerId(event));
    const handleCommandText = async (text, event, ctx) => {
    ...
    const cleanupRatedTurn = (turn, reactedKey, reactedId) => {
    ...
    const rememberPoll = (result, turn, event, ctx) => {
    ...
    const handlePollVote = async (event, ctx) => {
      ...
      const turn = pollId && capturedPollIds.get(pollId) || (messageId && !ambiguousMessageIds.has(messageId) ? capturedMessageIds.get(messageId) : void 0);
    ...
    const handleReactionRating = async (event, ctx) => {
      ...
      const turn = capturedMessages.get(reactedKey) || (reactedId && !ambiguousMessageIds.has(reactedId) ? capturedMessageIds.get(reactedId) : void 0);
      if (!turn || !canRateTurn(turn, event, ctx, cfg)) return false;
    ...
    const payloadFor = (rating, pendingKey) => `reinforceclaw:${rating}:${pendingKey}`;
    const sendFeedbackPrompt = async (turn, event, ctx) => {
      ...
      const accountId = accountIdOf(event, ctx) || void 0;
      const replyTo = outboundMessageIdOf(event, ctx?.message, ctx?.callback) || void 0;
    ...
    on(["message_received"], async (event, ctx) => {
    ...
    on(["message_sending", "message_sent"], async (event, ctx) => {
    ...
    const interactiveCommand = (ctx) => {
      const raw = String(
        ctx?.callback?.payload ?? ctx?.callback?.data ?? ctx?.interaction?.payload ?? ctx?.interaction?.data ?? ctx?.interaction?.value ?? ctx?.interaction?.values?.[0] ?? ""
      );
      let data = raw;
      try {
    ...
    const registerInteractive = (channel) => {
      try {
        api.registerInteractiveHandler({
          ...
          handler: async (ictx) => {
            ...
            const turn = pendingKey ? capturedPendingKeys.get(pendingKey) : void 0;
    ...
    const registerRateCommand = (name) => api.registerCommand({
      name,
      acceptsArgs: true,
      requireAuth: true,
